...
rutas.get('/reserver/:id', async (req, res) => {
    ...
    const servicesReserva =  await pool.query('select nombre, costo, fecha_reserva, hora_reserva, id_reserva, reserva_servicio.id from servicio inner join reserva_servicio on reserva_servicio.id_servicio = servicio.id where id_reserva = ?', [id_reserva]);
